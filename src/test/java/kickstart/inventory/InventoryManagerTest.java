...
public class InventoryManagerTest {
	...
	public void testAddArticle() {
		...
		assertThat(manager.getInventory().findByProduct(p).isPresent())
				.as("InventoryManager" + "should add an article to the inventory in method addArticle").isTrue();
	}
	...
	public void testHasSufficientQuantity() {
		...
		assertThat(manager.hasSufficientQuantity(p, Quantity.of(2, Metric.UNIT)))
				.as("InventoryManager" + "should return true when  the asked amount is lower than the current amount")
				.isTrue();

		assertThat(manager.hasSufficientQuantity(p, Quantity.of(50, Metric.UNIT)))
				.as("InventoryManager" + "should return false when  the asked amount is higher than the current amount")
				.isFalse();

		assertThat(manager.hasSufficientQuantity(q, Quantity.of(10, Metric.UNIT)))
				.as("InventoryManager" + "should return false when the article isn't present").isFalse();

		try {
			manager.hasSufficientQuantity(p, Quantity.of(2, Metric.LITER));
		} catch (Exception e) {
			assertThat(e).as("Manager should throw an IllegalArgumentException" + " when the metric isn't Metric.Unit")
	...
	public void testDecreaseQuantity() {
		...
		assertThat(manager.getInventory().findByProduct(newPart).get().getQuantity().getAmount()
				.equals(before.getAmount())).as(
						"InventoryManager shouldn't decrease the quantity if the asked quantity is greater than the current quantity ")
						.isTrue();

		manager.decreaseQuantity(newPart, before.subtract(Quantity.of(1, Metric.UNIT)));

		assertThat(manager.getInventory().findByProduct(newPart).get().getQuantity().getAmount()
				.compareTo(Quantity.of(1, Metric.UNIT).getAmount()) == 0).as(
						"InventoryManager should decrease the quantity if the asked quantity is less to the current quantity.")
						.isTrue();
	}
	...
	public void testIsPresent() {
		...
		assertThat(manager.isPresent(q)).as("InventoryManager should return false if an article isn't present ")
				.isFalse();

		assertThat(manager.isPresent(p.getId())).as("InventoryManager should return true if an article is present")
				.isTrue();

		assertThat(manager.isPresent(q.getId())).as("InventoryManager should return false if an article isn't present ")
				.isFalse();
	}
	...
	public void testUpdate() {
		...
		manager.getTime().forward(Interval.from(accountancy.getTime())
				.to(accountancy.getTime().plusDays(manager.getReorderTime())).getDuration());

		manager.update();

		assertThat(before.add(Quantity.of(5, Metric.UNIT)).getAmount()
				.compareTo(manager.getInventory().findByProduct(p).get().getQuantity().getAmount()) == 0)
