...
public class InventoryControllerTest {
	...
	public void setUp() {
		...
		controller.getManager().getInventory().save(new ReorderableInventoryItem(part1, Quantity.of(part1Amount)));
		controller.getManager().getInventory().save(new ReorderableInventoryItem(part2, Quantity.of(part2Amount)));
	}

	@Test
	@Transient
	public void testPublicAccess() throws Exception {
		mvc.perform(get("/inventory")).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, endsWith("/login")));

		mvc.perform(get("/reorders")).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, endsWith("/login")));

		mvc.perform(get("/inventory/update")).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, endsWith("/login")));

		mvc.perform(get("/reorder/" + part1.getId().toString())).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, endsWith("/login")));

		mvc.perform(get("/reorder/abc")).andExpect(status().isFound())
				.andExpect(header().string(HttpHeaders.LOCATION, endsWith("/login")));
	}
	...
	public void testInventoryView() throws Exception {
		...
		// See:
		// https://stackoverflow.com/questions/22527978/how-do-i-unit-test-spring-security-preauthorizehasrole
		RequestBuilder request = get("/inventory").with(user("boss").roles("EMPLOYEE"));

		Map<String, Object> model = mvc.perform(request).andExpect(status().isOk())
				.andExpect(model().attributeExists("inventoryItems")).andReturn().getModelAndView().getModel();
	...
	public void testReorderView() throws Exception {
		...
		Map<String, Object> model = mvc.perform(request).andExpect(status().isOk())
				.andExpect(model().attributeExists("reorders")).andReturn().getModelAndView().getModel();
	...
	public void testShowReorder() throws Exception {
		...
		RequestBuilder request = get("/reorder/" + part2.getId().toString()).with(user("boss").roles("EMPLOYEE"));

		mvc.perform(request).andExpect(status().isOk()).andExpect(model().attributeExists("form"))
	...
	public void testReorder() throws Exception {
		...
		RequestBuilder request = post("/reorder/" + part2.getId().toString()).with(user("boss").roles("EMPLOYEE"))
				.param("amount", Integer.valueOf(allowedReorderAmount).toString()).with(csrf());
		...
		for (LocalDateTime ldt : item.getReorders().keySet()) {
			assertThat(item.getReorders().get(ldt).getAmount()).as("Reorder should add the right amount")
		...
		request = post("/reorder/" + part1.getId().toString()).with(user("boss").roles("EMPLOYEE"))
				.param("amount", Integer.valueOf(illegalReorderAmount).toString()).with(csrf());

		mvc.perform(request).andExpect(status().isOk()).andExpect(model().attributeExists("form", "id", "name"))
				.andExpect(view().name("reorder"));
	...
	public void testInventoryUpdate() throws Exception {
		ReorderableInventoryItem item = controller.getManager().getInventory().findByProductIdentifier(part1.getId())
				.get();
		...
		assertThat(controller.getManager().getInventory().findByProductIdentifier(part1.getId()).get().getQuantity()
				.getAmount()).as("Update should increase the amount of updated items")
