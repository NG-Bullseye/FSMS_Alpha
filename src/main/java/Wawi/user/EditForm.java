...
import Wawi.validation.isRegistered;
...
interface EditForm {
	...
	@Size(max = 255, message = "Eingabe zu lang!")
	// @Pattern(regexp = "[^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$]", message =
	// "Bitte geben sie einen gültigen Nachname ein!")
	String getLastname();
	...
	@Size(max = 255, message = "Eingabe zu lang!")
	// @Pattern(regexp = "[CDATA[^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{4,32}$]]",
	// message = "Passwort zu leicht! 4-32 Zeichen mit: Zahl, Groß-,
	// Kleinbuchstaben")
	String getPassword();
